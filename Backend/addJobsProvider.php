...
if (isset($_SESSION['user'])) {
    ...
    if ($user_role === "service_provider") {
        ...
        $minSalary       = isset($form['minSalary']) ? floatval($form['maxSalary']) : 0;
        $maxSalary       = isset($form['maxSalary']) ? floatval($form['minSalary']) : 0;
        // Validation
        $errors = [];
        ...
        if ($minSalary <= 0) $errors[] = "Salary must be a positive number.";
        if ($maxSalary <= 0 && $minSalary <= $maxSalary ) $errors[] = "Salary must be a positive number.";
        ...
        $addJob=new Job();
        $response=$addJob->addJob($user_id, $title, $description, $requirements, $benefits, $location, $job_type, $expiryDate, $minSalary,$maxSalary);
